...
@router.post("/", dependencies=[Depends(login_required)],response_model=DataResponse[CreateNewGroupResponse])
...
@router.get("/", dependencies=[Depends(login_required)], response_model=DataResponse[GetGroupsByUserIdResponse])
...
def get_private(
    ...
    try:
        res = group_service.get_private(
            user=user, friend_id=friend_id, db=db
        )
...
@router.get("/{group_id}", dependencies=[Depends(login_required)], response_model=DataResponse[GetGroupByGroupIdResponse])
def get_group_by_group_id(
    ...
    try:
        res = group_service.get_group_by_group_id(
            user=user, group_id=group_id, db=db
        )
        return DataResponse().success_response(data=res)
    except Exception  as e:
...
@router.put("/{group_id}", dependencies=[Depends(login_required)], response_model=DataResponse[UpdateGroupSimpleResponse])
def update_group(
    group_id: int,
...
@router.post("/groups/{id}/members", dependencies=[Depends(login_required)], response_model=DataResponse[Any], deprecated=True)
